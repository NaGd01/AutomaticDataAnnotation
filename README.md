...
Data spaces have emerged as versatile platforms for managing and analyzing data from diverse sources, facilitating integration, collaboration, and governance across organizations and systems. By providing access to high-quality curated data sets, data spaces play a pivotal role in supporting foundation models. However, efficient data spaces require annotated data to enable such large-scale data processing. Manual annotation has challenges related to scalability, costs, and bias. To address this, we introduce SEADA, a semantic automatic data annotation approach for large-scale data spaces. SEADA leverages the semantic Data Product Description Object (DPDO) and combines state-of-the-art meta data embedding models in an annotation pipeline. Supporting structured data sets, SEADA automatically annotates open-domain data sets. We illustrate the approach by means of an annotator service, evaluating it through competency questions and multiple data sets from various domains.
